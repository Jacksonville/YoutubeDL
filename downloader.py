...
class YouTube:
    def __init__(self):
        self.version_file = os.path.join(os.getcwd(),'youtube-dl.version')
    ...
    def video_download(self, video_link, outputdir):
        command = '''youtube-dl --output "{0}/%(title)s.%(ext)s" "{1}"'''.format(outputdir, video_link)
        print command
        os.system(command)
...
class MainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self, parent=None):
        ...
        #some test data
        #self.txtbxUrl.setText('http://www.youtube.com/watch?v=AgFeZr5ptV8')
        #self.rbUrl.toggle()
        #self.source_rb_changed()
        #self.lblOutputDir.setText(os.getcwd())
    ...
    def setup_form(self):
        ...
        self.txtOutputText.setVisible(False)
    ...
    def youtube_dl_version_check(self):
        ...
        if new_version and new_version != 'Unable to get version':
            ...
            if res == QMessageBox.StandardButton.Yes:
                self.yt.download_file(new_version)
...
def main():
    ...
    app.setWindowIcon(QIcon('ytd.ico'))
    frame = MainWindow()
    frame.show()
    app.exec_()
